...
contract ADKToken {
    ...
    mapping (address => string) public known_addresses;
    ...
    AGSClaim public AGSClaimContract; // contract to claim airdropped AGS
    // CONSTRUCTOR
    constructor() {
        
        // build Genesis contract structure
        adkgo_genesis_address = msg.sender;
        AGSClaimContract = new AGSClaim(adkgo_genesis_address, address(this));
    ...
    function meshTransaction(string memory _meshaddr, int256 _value) public onlyADKTransactionContract requireValidADKAddress(_meshaddr) {
        ...
        known_addresses[addr] = _meshaddr; // store known mapping
    ...
    function registerAddress(string memory _meshaddr) public requireValidADKAddress(_meshaddr) {
        address addr = AZ9_TO_ADDR(_meshaddr);
        known_addresses[addr] = _meshaddr; // store known mapping
    }
