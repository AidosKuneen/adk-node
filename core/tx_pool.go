...
import (
	...
	"github.com/aidoskuneen/adk-node/common/prque"
	"github.com/aidoskuneen/adk-node/consensus/misc"
	"github.com/aidoskuneen/adk-node/core/state"
	"github.com/aidoskuneen/adk-node/core/types"
	"github.com/aidoskuneen/adk-node/event"
	"github.com/aidoskuneen/adk-node/log"
...
const (
	...
	txMaxSize = 4 * 4 * txSlotSize // 512 KB
)
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	//ADKGO ensure that the minimum GAS PRICE for contract creation is sufficient (adkphoenix)
	//
	// Note: 2021/11 Not needed any more as we use GAS recycle instead
	//
	// if (tx.To() == nil) { // contract creation
	//     minGasPrice := new(big.Int).SetUint64(params.TxContractCreationMinGasPrice)
	// 	if ( tx.GasPrice().Cmp(minGasPrice) < 0 && bytes.Compare(from.Bytes(),common.HexToAddress(params.TxContractCreationGenesisAccount).Bytes()) != 0){
	// 		return ErrInsufficientGasPriceForContractCreation
	// 	}
	// }

	//ADKGO only allow MESH transactions executed via PoW API to be "free"
  ...
  if (bytes.Compare(from.Bytes(),common.HexToAddress(params.TxContractCreationGenesisAccount).Bytes()) == 0){
	...
	if (tx.To() != nil) && (tx.Data() != nil) {
		 ...
		 if (  ( bytes.Compare(tx.To().Bytes(),common.HexToAddress(params.TxAGSClaimContract).Bytes()) == 0 ) &&
				...
				dataLen := 0
				validchars := "ABCDEFGHIJKLMNOPQRSTUVWXYZ9"
				allNines := true
				for idx := 0; idx < len(data); idx++{
					if data[idx] != 0 && strings.Contains(validchars,string(data[idx:idx+1])) {
						dataLen++
						if (string(data[idx:idx+1])!="9") {
							allNines = false
						}
					} else {
						break
				...
				if allNines || dataLen == 0 || dataLen % 2673 != 0 {
					return ErrTransactionInvalid
				}
				data = data[0:dataLen]
				// ok by now we know we have a valid tryte. so now letch check for each if the PoW has been Done
				for tidx := 0; tidx < len(data); tidx += 2673 {
					transactionData := data[tidx:tidx+2673]
					transactionDataTrytes,err:=gadk.ToTrytes(string(transactionData))
					if err != nil || !strings.HasSuffix(string(transactionDataTrytes.Hash()), "99999"){ // 15 trits difficulty
						return ErrInsufficientPoW
					}
				}
				// and finally check that we ahvent seen this exact transaction recently, again to prevent spam
				// otherwise raise ErrTransactionRecent

				// first clear expired ones
				currTime := time.Now().Unix()  // time in seconds since ep
				for dt , unixtime := range seen_trytes {
					if (unixtime < currTime - 60 ){ // allow same transaction again after 1 min
						delete(seen_trytes, dt)
					}
				}
				currTime = time.Now().Unix()  // time in seconds since ep
				if (seen_trytes[string(data)]==0){
					  seen_trytes[string(data)] = currTime
			  		return nil // all transactions OK
				} else {
					return ErrTransactionRecent
				}
		 }
	}
